...
const FROM = process.env.MAIL_FROM ?? 'Obstresla <no-reply@obstresla.pro>';

export const APP_URL = process.env.APP_URL
  ?? (process.env.NODE_ENV === 'production' ? 'https://obstresla.pro' : 'http://localhost:5173');
...
const layout = (title: string, body: string, cta: { url: string; label: string }) => `
  <div style="max-width:480px;margin:0 auto;padding:32px 24px;font-family:system-ui,sans-serif;color:#1a1d29">
    <div style="font-size:20px;font-weight:800;margin-bottom:16px">⚡ Obstresla</div>
...
export function verifyEmailHtml(token: string): string {
  ...
  return layout('Confirma tu correo',
    'Gracias por crear tu cuenta en Obstresla. Confirma tu dirección de correo para poder recuperar tu cuenta si olvidas la contraseña.',
...
export function resetPasswordHtml(token: string): string {
  ...
  return layout('Restablece tu contraseña',
    'Recibimos un pedido para restablecer tu contraseña de Obstresla. El enlace vence en 1 hora y solo puede usarse una vez.',
