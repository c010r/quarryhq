...
app.post('/api/billing/cancel', requireAuth, h(async (req: AuthedRequest, res) => {
  ...
  if (provider === 'mercadopago') {
    ...
    try {
      ...
      // Cancelamos el cobro en MP pero NO degradamos el plan: el usuario sigue
      // con Premium hasta que venza el período ya pagado (userPlan() lo degrada
      // automáticamente al llegar a premium_until).
      return res.json({ ok: true, message: 'Suscripción cancelada. Conservás Premium hasta que finalice tu período actual.' });
    } catch (err: any) {
